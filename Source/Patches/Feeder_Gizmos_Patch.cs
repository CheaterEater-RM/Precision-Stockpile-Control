...
{
    // Appends the feeder gizmos to stockpile zones and storage buildings (shelves + mod storage that
    // extends Building_Storage). Both seams are the vanilla GetGizmos overrides confirmed in source.
    // Gated to a single selection so click-to-target is unambiguous.
