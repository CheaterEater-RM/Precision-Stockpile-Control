...
namespace PrecisionStockpileControl
{
    ...
    // queued items are still spawned in their source. The shared PscCarriedSourceCapture helper snapshots each
    // feeder-sourced item's origin into PscCarriedSourceTracker, keyed by (pawn, def) count segments so the
    // later merge can't defeat it; the restore reads it back during the unload store-search
    // (PscAdmissionIndex.TryFeederReject). Guarded by Prepare()/TargetMethod via reflection — no compile- or
    // load-time reference to PUAH.
    [HarmonyPatch]
    public static class PickUpAndHaul_HaulToInventory_Capture_Patch
    {
        ...
        // PUAH queues its pickup targets in targetQueueA; the capture body is shared with HD's bulk-gather patch.
        public static void Postfix(JobDriver __instance, bool __result)
            => PscCarriedSourceCapture.CaptureQueue(__instance, __result, TargetIndex.A);
    }
}
